...
| `local-code-agent` pytest (105 tests, newly built during this audit) | **105/105 passed** — `tests/test_db_tools.py` (46), `tests/test_permissions.py` (32, parametrized across both permission-manager implementations), `tests/test_indexer.py` (27) |
...
3. **`local-code-agent`'s new test suite (§8.0 below) does not yet cover the agent loop, tool
   execution, or context management end-to-end** — it covers the pure/deterministic layers
   (`db_tools`, permissions, AST indexing) thoroughly, but `agent/tools.py`'s `ToolRegistry`,
   `agent/context_manager.py`'s `ContextManager`, and the full `GuiAgentLoop`/`agent/api_server.py`
   integration are still only verified by the ad-hoc, disposable scripts used throughout this audit
   and the sessions before it — real coverage, but not persistent, re-runnable coverage.
...
`pytest` suite was built and added: **105 tests across three files**
(`tests/test_db_tools.py`, `tests/test_permissions.py`, `tests/test_indexer.py`), covering:
...
All 105 pass; `pyflakes` clean on the new files (one self-introduced unused import was caught and
fixed the same way as everything else in this audit); the existing `telegram_agent_bot` suite (23
tests) and full bridge integration test re-confirmed unaffected. `pytest` added to
`requirements.txt` as a clearly-separated, still-optional dev dependency (distinct from the
existing `pytest` entry that powers the `run_tests` *tool* for the user's own projects, which is a
different purpose and shouldn't be conflated with it).

**Still not covered, and worth doing next** (see §7.3): `ToolRegistry`, `ContextManager`, and the
full agent-loop integration. The stubbed-`OllamaClient` pattern used throughout this audit's ad-hoc
verification (`test_harness.py`) is a proven, working approach for this — formalizing it into
`tests/test_integration.py` is the natural next step, exercising a real multi-turn tool-use
sequence against the real `ToolRegistry`/`ContextManager`/permission manager without needing a
real Ollama server.
