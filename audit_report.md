...
| `local-code-agent` pytest (113 tests, newly built during this audit) | **113/113 passed** — `tests/test_db_tools.py` (46), `tests/test_permissions.py` (32, parametrized across both permission-manager implementations), `tests/test_indexer.py` (27), `tests/test_integration.py` (8, real `ToolRegistry`/`ContextManager`/`GuiAgentLoop` together). Re-run 3× including reversed file order to confirm no ordering flakiness from the shared global event queue |
...
3. **`agent/api_server.py` (the FastAPI bridge) itself still has no persistent test coverage**,
   even after §8.0's integration suite addition — that suite covers `ToolRegistry`/
   `ContextManager`/`GuiAgentLoop` together (the shared core), but not the bridge's own REST/WS
   layer (concurrency guard, pause/resume/stop, event-to-`WSEvent` translation). That layer was
   verified extensively during earlier work on this codebase and re-confirmed working during this
   audit's regression passes, but only via the same disposable, ad-hoc script pattern - not
   anything `pytest` would catch on a future change. The real `AgentAPIClient`-based verification
   pattern used throughout this audit (start the real bridge as a subprocess with a stubbed LLM,
   drive it with the bot's own typed client) is proven and ready to formalize, following the same
   approach `tests/test_integration.py` just did for the shared core underneath it.
...
`pytest` suite was built and added: **113 tests across four files**
(`tests/test_db_tools.py`, `tests/test_permissions.py`, `tests/test_indexer.py`,
`tests/test_integration.py`), covering:
...
- `agent/tools.py` + `agent/context_manager.py` + `gui/agent_loop_gui.py` integration (8 tests,
  ...
  message-count reduction and summary creation. Required careful test isolation given
  `gui.events` uses a module-level global queue shared across the whole test process - an
  autouse fixture drains it before and after every test in the file; confirmed non-flaky by
  running the full 113-test suite three times, including once in fully reversed file order.

All 113 pass; `pyflakes` clean on every new file (two more self-introduced unused imports were
caught along the way and fixed the same way as everything else in this audit, including in the
audit's own new test code — the standard applied throughout wasn't relaxed for it); the existing
`telegram_agent_bot` suite (23 tests) and full bridge integration test re-confirmed unaffected.
`pytest` added to `requirements.txt` as a clearly-separated, still-optional dev dependency
(distinct from the existing `pytest` entry that powers the `run_tests` *tool* for the user's own
projects, which is a different purpose and shouldn't be conflated with it).
