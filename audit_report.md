...
| `local-code-agent` pytest (128 tests, newly built during this audit) | **128/128 passed** — `tests/test_db_tools.py` (46), `tests/test_permissions.py` (32, parametrized across both permission-manager implementations), `tests/test_indexer.py` (27), `tests/test_integration.py` (8, real `ToolRegistry`/`ContextManager`/`GuiAgentLoop` together), `tests/test_api_server.py` (15, the real FastAPI bridge via `TestClient`). Re-run multiple times including reversed file order to confirm no ordering flakiness from the two files' shared global state |
| Full syntax check, both repos | **All files valid** |
| `pyflakes`, both repos | **0 findings** in `telegram_agent_bot` and in `local-code-agent`'s new `tests/`; **1 remaining note** in `local-code-agent`'s `agent/` (see below) |
...
1. **The `union-attr` mypy noise in `telegram_agent_bot`'s handlers** (~50 findings, §2.5) is real
...
2. **No CI/automated pipeline** wires any of `pyflakes`/`mypy`/`bandit`/`pytest` into a pre-commit
   or CI check for either project — everything run in this audit was invoked manually.
3. **`agent/api_server.py` uses FastAPI's `@app.on_event("startup")`, deprecated in favor of
   lifespan event handlers** — noticed as a `DeprecationWarning` while building §8.0's new bridge
   test suite, not something introduced by this audit. Left unchanged: migrating it is a genuine
   behavior-preserving refactor of production request-handling code, not a test-writing task, and
   carries its own risk that deserves its own focused pass rather than a drive-by fix bundled into
   test-suite work. `local-code-agent`'s `agent/api_server.py` remains fully functional today;
   `on_event` still works, it's simply the older of two supported patterns.

---

## 8. Suggested Future Improvements

**0. [Addressed during this audit, as a direct continuation of it]** — `local-code-agent` had no
formal, persistent test suite; this was flagged as the single most significant gap found. A real
`pytest` suite was built and added: **128 tests across five files**
(`tests/test_db_tools.py`, `tests/test_permissions.py`, `tests/test_indexer.py`,
`tests/test_integration.py`, `tests/test_api_server.py`), covering:
- `agent/db_tools.py` (46 tests) — real SQLite databases throughout, not mocks. Notably includes
  a direct regression test for the exact behavior the source code's own comment flags as
  "verified this the hard way - do not remove" (the `BEGIN`-before-DDL fix that makes `dry_run`
  genuinely safe for `DROP TABLE`/`ALTER TABLE`, not just `DELETE`/`UPDATE`), and a regression
  test for the `get_schema` bug fixed in §2.2 of this same audit.
- `agent/permissions.py` + `agent/auto_permissions.py` (32 tests) — a parametrized fixture runs 10
  shared safety tests (allowed_roots enforcement, path-traversal resistance, hard_denylist
  matching) against *both* implementations identically, so the two can never silently drift out
  of sync with each other; plus implementation-specific tests for the interactive approval flow's
  trust-scoping rules (`always` vs `session`, read trust not implying write trust) and the
  auto-approve manager's action-logging visibility guarantee.
- `agent/indexer.py` (27 tests) — `extract_python_symbols`, `find_containing_symbol`,
  `build_symbol_table`, and `build_dependency_graph`, tested against real files on disk via
  `tmp_path`, including the "innermost symbol wins" rule, honest name-collision disclosure, and
  correct forward/reverse dependency-graph inversion.
- `agent/tools.py` + `agent/context_manager.py` + `gui/agent_loop_gui.py` integration (8 tests,
  `tests/test_integration.py`) — the real `ToolRegistry`, `ContextManager`,
  `AutoApprovePermissionManager`, and `GuiAgentLoop` running together exactly as in production,
  only the LLM call itself stubbed (a scripted, multi-step tool-use sequence via pytest's
  `monkeypatch`, safely auto-reverting per test, unlike the ad-hoc harness's permanent
  module-level patch). Verifies real, observable side effects rather than mocked call arguments:
  a real file write landing on disk (read back afterward, not inferred from a tool result string),
  real plan-status transitions, real focus-file tracking, a tool-call error producing a graceful
  result instead of crashing the loop, scratchpad/plan persistence surviving a *second, independent*
  `ContextManager` instance loading the same path, the parallel tool-execution path specifically
  (with an explicit in-test assertion guarding against silently degrading to the sequential
  fallback if that detection logic ever changes), and `ContextManager.compact()`'s real
  message-count reduction and summary creation.
- `agent/api_server.py` (15 tests, `tests/test_api_server.py`) — the FastAPI bridge's own REST/WS
  layer, via `TestClient` against the real app (real routing, real background thread, real
  event-pump task), not a mock ASGI transport. Directly re-verifies the exact concurrency bug found
  and fixed earlier in this audit: two prompts fired close together, with the same realistic-delay
  technique that was needed to reproduce it the first time (a zero-delay stub finishes too fast to
  ever race). Also covers pause/resume/stop against real project state (including stopping a turn
  genuinely in flight, confirmed by the timing math making natural completion impossible before the
  stop lands), file browsing plus the path-traversal defense, 404 handling for unknown project IDs,
  `/system` and `/projects` field shapes, and a real event received over an actual `TestClient`
  WebSocket connection - not an in-process shortcut, the genuine ASGI WebSocket route handler,
  `_ws_clients` registration, and `_broadcast()` call. Required a `tests/conftest.py` to locate the
  sibling `telegram_agent_bot` repo `agent/api_server.py` imports directly from (matching the
  launch scripts' existing approach), with a graceful `pytest.importorskip` if it's genuinely
  absent rather than a confusing collection error; and its own state-isolation fixture, since this
  module's globals (`_session`, `_project`, `_active_thread`, etc.) were written for one
  long-running process, not repeated isolated test runs.

Both new integration-style files required real test isolation given `gui.events`' module-level
global queue and `agent/api_server.py`'s own module-level globals are shared across the whole test
process - confirmed non-flaky by running the full 128-test combined suite multiple times, including
in fully reversed file order.

All 128 pass; `pyflakes` clean on every new file (three self-introduced unused imports were
caught along the way across this whole test-writing effort and fixed the same way as everything
else in this audit, including in the audit's own new test code — the standard applied throughout
wasn't relaxed for it); the existing `telegram_agent_bot` suite (23 tests) and full bridge
integration test re-confirmed unaffected. `pytest` added to `requirements.txt` as a
clearly-separated, still-optional dev dependency (distinct from the existing `pytest` entry that
powers the `run_tests` *tool* for the user's own projects, which is a different purpose and
shouldn't be conflated with it).

**1. Wire `pyflakes`, `mypy`, and `bandit` into a simple pre-commit hook or CI step** for both
...
**2. Consider narrowing the ~50 `union-attr` findings with light `assert query is not None`-style
...
**3. Full permission-approval parity for the bridge** (a real `PERMISSION_REQUEST` WS event +
