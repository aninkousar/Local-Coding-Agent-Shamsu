...
def get_schema(db_path: str, db_type: str = "sqlite") -> str:
    ...
    try:
        ...
        # In practice unreachable - connect() above already raises DBError for any
        # db_type other than sqlite/postgres/mysql, so execution never reaches here
        # with an unhandled value. Kept as an explicit, safe fallback rather than
        # relying on that as an implicit invariant between two separate functions -
        # this function's own -> str signature should never silently return None.
        raise DBError(f"Unknown db_type '{db_type}'. Use 'sqlite', 'postgres', or 'mysql'.")
    finally:
        conn.close()
