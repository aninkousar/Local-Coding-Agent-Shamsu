(() => {
  ...
  const workingBar = document.getElementById("working-bar");
  ...
  function onPlanUpdate(steps) {
    ...
    const hasAnyFeature = steps.some((s) => s.feature);
    let seenFeature = Symbol("unset"); // sentinel - never equals a real feature name or null
    let stepNum = 0;
    steps.forEach((s) => {
      if (hasAnyFeature && s.feature !== seenFeature) {
        seenFeature = s.feature;
        const header = document.createElement("div");
        header.className = "plan-feature-header";
        header.textContent = s.feature || "(ungrouped)";
      ...
      if (hasAnyFeature) stepEl.classList.add("plan-step-grouped");
      const icon = status === "completed" ? "✔" : status === "in_progress" ? "▶" : "○";
      stepEl.querySelector(".plan-step-icon").textContent = icon;
      stepEl.querySelector(".plan-step-desc").textContent = `${stepNum}. ${s.description || ""}`;
