@echo off
REM Local Code Agent launcher for cmd.exe
REM Double-click this file, or drag a project folder onto it, or run it
REM with a path argument: launch-agent.bat C:\path\to\my-project
...
start "Local Code Agent" cmd /k call "%AGENTDIR%.venv\Scripts\activate.bat" ^&^& cd /d "%PROJDIR%" ^&^& local-agent
