...
call update_plan FIRST with a short, concrete breakdown of the segments you'll tackle in order. \
Each step must be small enough to finish in one or two tool calls - one file, one function, one \
route, one migration. Not "build the backend" (too broad - that's several steps hiding in one) \
but "create the Product model", "add the /api/products route", "seed 3 sample products". If a \
step needs the word "and" to describe it, it's really two steps - split it.
18. Work through the plan ONE step at a time. After finishing a step, call update_plan again \
immediately - marking that step [x] and the next one [~] - before starting any other tool call \
for the next step. Don't silently finish three steps and only update the plan at the end; the \
point is that the user can see real progress as it happens, not a wall of changes all at once.
