...
def voice_confirm_keyboard() -> InlineKeyboardMarkup:
    """Shown after a voice note is transcribed - the pending transcript and
    whether it's a new-project or follow-up prompt live in
    context.user_data (see bot/handlers/prompt.py), not in callback_data,
    since there's only ever one pending transcript per chat at a time - no
    id/arg needed in these callback_data strings."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ Send as-is", callback_data="voice:send")],
        [InlineKeyboardButton("✏️ Send as text instead", callback_data="voice:edit")],
        [InlineKeyboardButton("❌ Discard", callback_data="voice:discard")],
    ])
